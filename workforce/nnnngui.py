...
# Define status colors and their corresponding selected colors
status_colors = {
    'running': 'lightblue',
    'run': 'lightcyan',
    'ran': 'lightgreen',
...
light_to_regular_colors = {
    'lightblue': 'blue',
    'lightcyan': 'cyan',
    'lightgreen': 'green',
    'lightcoral': 'coral',
    'gray': 'darkgray',  # Default node color when selected
    'lightgray': 'gray'  # Default edge color when selected
}
...
def create_stylesheet():
    stylesheet = [
        {
            ...
            'style': {
                ...
                'text-wrap': 'wrap',
                'background-color': 'data(color)',  # Use data(color) for node color
            },
        },
        {
            'selector': 'node:selected',
            'style': {
                'background-color': 'data(selected_color)',  # Use data(selected_color) for selected nodes
            },
        },
        {
            'selector': 'edge',
            'style': {
                ...
                'line-color': 'data(color)',  # Use data(color) for edge color
                'target-arrow-color': 'data(color)',  # Use data(color) for edge arrow color
            },
        },
        {
            'selector': 'edge:selected',
            'style': {
                'line-color': 'data(selected_color)',  # Use data(selected_color) for selected edges
                'target-arrow-color': 'data(selected_color)',
            },
        },
    ]
    ...
    for status, color in status_colors.items():
        regular_color = light_to_regular_colors.get(color, color)
        stylesheet.extend([
            {
    ...
    # Default styles for nodes and edges without status
    stylesheet.extend([
        {
            'selector': 'node[!status]',
            'style': {
                'background-color': 'gray',  # Default node color
                'selected_color': 'darkgray',  # Default selected node color
            },
        },
        {
            'selector': 'edge[!status]',
            'style': {
                'line-color': 'lightgray',  # Default edge color
                'target-arrow-color': 'lightgray',  # Default edge arrow color
                'selected_color': 'gray',  # Default selected edge color
            },
        }
    ])
...
def load(pipeline_file):
    ...
    for element in elements:
        if 'source' in element and 'target' in element:
            ...
            selected_color = light_to_regular_colors.get(color, color)
            updated_element = {
                'data': {
                    'source': element['source'],
                    'target': element['target'],
                    'id': element.get('id', f"{element['source']}-{element['target']}"),
                    'status': status,
                    'color': color,
            ...
            updated_element = {
                'data': {
                    ...
                    'id': element.get('id', str(len(dash_cytoscape_data) + 1)),
                    'status': status,
                    'color': color,
                    'selected_color': selected_color,
                },
...
def add_node(elements, txt_node):
    new_node = {
        'data': {
            ...
            'color': 'gray',  # Default color for new nodes
            'selected_color': 'darkgray',  # Selected color for new nodes
        }
    }
    ...
    for i in range(len(selected_nodes) - 1):
        ...
        new_edge = {
            'data': {
                ...
                'color': 'lightgray',  # Default edge color
                'selected_color': 'gray',  # Selected edge color
            }
        }
