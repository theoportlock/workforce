...
# Global variable if you still need it for backward compatibility
global_pipeline_file = None

def Gui(pipeline_file=None):
    global global_pipeline_file
    global_pipeline_file = pipeline_file
    app = Dash(__name__)
    app.title = "Workforce"
...
def create_layout():
    return html.Div([
        ...
        html.Div([
            dcc.Input(id='txt_node', value='echo "Input bash command"', type='text',
...
def register_callbacks(app):
    ...
    def modify_network(contents, add_clicks, remove_clicks, connect_clicks, update_clicks,
                       txt_node, filename, last_modified, elements, selected_nodes, selected_edges):
        global global_pipeline_file
        if ctx.triggered_id == 'upload-data' and contents is not None:
            global_pipeline_file = filename
    ...
    # Callback for updating the filename store when a file is uploaded.
    @app.callback(
        Output('pipeline-store', 'data'),
        [Input('upload-data', 'filename')],
        prevent_initial_call=True
    )
    def update_pipeline_store(filename):
