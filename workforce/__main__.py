...
def _main_impl():
    ...
    web_p.add_argument(
        "--foreground", "-f", action="store_true", help="Reserved for parity with gui"
    )
    web_p.add_argument(
        "--server-url",
        help="Server URL (overrides WORKFORCE_SERVER_URL, default http://127.0.0.1:5049)",
    ...
    def _web(args):
        _ = args.foreground  # Reserved for parity with gui.
        parsed = (
            utils.parse_workspace_url(args.url_or_path) if args.url_or_path else None
        )
    ...
    server_p = subparsers.add_parser(
        "server", help="Manage the single machine-wide server"
    )
    ...
    sp.add_argument(
        "--port", type=int, default=5049, help="Port to bind to (default: 5049)"
    )
    sp.add_argument(
        "--log-dir", help="Directory for server.log (default: ~/.workforce)"
    )
    ...
    sp3.add_argument(
        "--server-url", help="Server URL to query (default: current running server)"
    )
    sp3.set_defaults(func=lambda args: server_cmd_list(args))

    # Add workspace registration
    sp4 = server_sub.add_parser(
        "add", help="Register a workspace path with the running server"
    )
    sp4.add_argument("path", help="Workfile path to register")
    sp4.add_argument(
        "--host", default=None, help="Server host (overrides WORKFORCE_SERVER_URL)"
    )
    sp4.add_argument(
        "--port",
        type=int,
        default=None,
        help="Server port (overrides WORKFORCE_SERVER_URL)",
    )
    ...
    def _server_add(args):
        ...
        registration = register_workspace(
            utils.resolve_server(server_url=server_url), ensure_workfile(args.path)
        )
