...
def register_callbacks(app):
    ...
    def view_pipeline(n_clicks):
        if global_pipeline_file:
            subprocess.run(f"wf view {global_pipeline_file}", shell=True)
    ...
    def run_pipeline(n_clicks):
        if global_pipeline_file:
            subprocess.run(f"wf run {global_pipeline_file}", shell=True)
        return 0  # Reset click count
